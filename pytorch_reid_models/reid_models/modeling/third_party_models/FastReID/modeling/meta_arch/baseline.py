...
class Baseline(nn.Module):
    ...
    def forward(self, batched_inputs, return_loss=False, overhaul=False, is_feat=False, is_s_to_t = False, is_incv4 = False):
        # 将学生特征输入到老师网络
        if is_s_to_t:
            outputs = self.heads(batched_inputs)
        ...
        if is_incv4:
            return features
        if is_feat:
            feats, feat = self.backbone.extract_feature(images, preReLU=True)
            return feats, feat
        if overhaul:
            if self.training:
                feats, feat = self.backbone.extract_feature(images, preReLU=True)
                outputs = self.heads(features)
                return outputs["cls_outputs"], outputs["features"], feats
