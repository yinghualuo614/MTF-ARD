...
class Test_Baseline(nn.Module):
    ...
    def forward(self, batched_inputs, return_loss=False, is_divide_feature = "正常执行", resnet = "18"):
        # 不从特征中提取鲁棒特征
        if is_divide_feature == "不从特征中提取鲁棒特征":
            ...
            if self.training:
                ...
                return outputs["cls_outputs"], outputs["features"], features
        # 将特征分为鲁棒特征
        elif is_divide_feature == "从特征中提取鲁棒特征":
            images = self.preprocess_image(batched_inputs)
            features, r_output = self.backbone(images, is_divide_feature = is_divide_feature, resnet = resnet)

            if self.training:
                outputs = self.heads(features)
                return outputs["cls_outputs"], outputs["features"], features, r_output
