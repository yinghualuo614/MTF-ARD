...
from ...modeling.meta_arch import META_ARCH_REGISTRY, Distiller
...
class DistillerOverhaul(Distiller):
    def __init__(self, cfg):
        ...
        s_channels = self.backbone.get_channel_nums() # 获得输入到学生网络resnet每层的channel数，s_channels = [64,128,256,512]

        for i in range(len(self.model_ts)):
            t_channels = self.model_ts[i].backbone.get_channel_nums() # 获得输入到教师网络resnet每层的channel数，t_channels = [256,512,1024,2048]
